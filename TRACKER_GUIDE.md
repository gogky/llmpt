...
**完全支持 IPv6！** 详细说明请查看：[IPv6_SUPPORT.md](./IPv6_SUPPORT.md)

- ✅ 双栈支持（同时 IPv4 和 IPv6）
- ✅ IPv6 Compact 格式（18 字节/Peer）
- ✅ 自动分离和返回 IPv4/IPv6 Peer 列表
- ✅ 兼容所有主流 BT 客户端
...
**IPv4 Peers** - `peers` 字段，每 6 字节表示一个 Peer：
...
**IPv6 Peers** - `peers6` 字段（BEP-0007），每 18 字节表示一个 Peer：
...
示例：
```
[2001:db8::1]:6881 -> 20 01 0D B8 00 00 00 00 00 00 00 00 00 00 00 01 1A E1
```
